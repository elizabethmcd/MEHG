...
bar_env = environment %>% ggplot(aes(x=reorder(code, freq), y=freq)) + geom_bar(stat="identity", fill=(phaColfunc(13))) + coord_flip() + scale_y_continuous(limits=c(0,215), expand = c(0, 0)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank())
bar_env 

# phyla plot
phylaColfunc = colorRampPalette(c("khaki1", "orchid4"))
bar_phyla = group %>% ggplot(aes(x=reorder(Group, freq), y=freq)) + geom_bar(stat="identity", fill=(phylaColfunc(18))) + coord_flip() + scale_y_continuous(limits=c(0,400), expand= c(0,0)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank())
bar_phyla

# reference isolate phyla 
references = metadata %>% filter(code=="Isolate")
reference_phyla = count(references, "Phyla")
bar_ref = reference_phyla %>% ggplot(aes(x=reorder(Phyla, freq), y=freq)) + geom_bar(stat="identity", fill=(phylaColfunc(8))) + coord_flip() + scale_y_continuous(limits=c(0,100), expand= c(0,0)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank())
bar_ref

# CP phyla
cp = metadata %>% filter(Group=="Candidate Phyla")
cp_phyla = count(cp, "Phyla")
bar_cp= cp_phyla %>% ggplot(aes(x=reorder(Phyla, freq), y=freq)) + geom_bar(stat="identity", fill=(phylaColfunc(21))) + coord_flip() + scale_y_continuous(limits=c(0,6), expand= c(0,0)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank())
bar_cp 
...
isolate_avgs = isolate_sizes %>% ggplot(aes(x=Group, y=size)) + geom_boxplot()
...
gc_meth_isolates = gc_isolates %>% ggplot(aes(x=Group, y=gc)) + geom_boxplot()
...
# stats
ggsave(file="figs/2019-09-02-all-genome-sizes.png", all_avgs_theme, width=10, height=7, units=c("cm"))
ggsave(file="figs/2019-09-02-all-gc.png", gc_meth_theme, width=10, height=7, units=c("cm"))
