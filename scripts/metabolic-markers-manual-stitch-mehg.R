...
markers = read.csv("results/metabolic_results/raw-metabolic-marker-results.csv")
metadata = read.csv("files/stats/quals/mehg-markers-metadata.csv", header=FALSE, stringsAsFactors = FALSE)
nitrogen = read.csv("results/metabolic_results/all-nitrogen-markers.csv")
mercury = read.csv("results/metabolic_results/mercury-markers.csv")
# column names
...
metals.plot = metals.melted %>% ggplot(aes(x=variable, y=(rev(group)), fill=value)) + geom_tile(color='black',size=0.5,aes(width=1, height=1)) + coord_fixed() +  scale_fill_gradient(low="grey85", high="midnightblue") + theme(panel.grid = element_blank(), panel.border = element_blank(), plot.margin = unit(c(0, 0, 0, 0), "cm"))
